            ...
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: 'my-ssh-key', keyFileVariable: 'SSH_KEY_FILE', usernameVariable: 'SSH_USER')]) {
                            ...
                            sh "chmod 600 \$SSH_KEY_FILE"
                            sh """
                            ssh -i \$SSH_KEY_FILE -o StrictHostKeyChecking=no -o ProxyCommand='ssh -i \$SSH_KEY_FILE -W %h:%p \$SSH_USER@51.44.82.249' \$SSH_USER@10.10.3.173 'sudo docker run -d --name backend -p 3000:3000 avengersa/backend:v6'
                            """
                        }
